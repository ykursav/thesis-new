...
##        cv2.destroyAllWindows()
    ...
    cpdef order_contour(self, points):
        ordered_points = np.zeros((4, 2), dtype = np.float32) #!!!!Why float? changed to int if there is any problem change to float32 again!!!!!
    ...
    cpdef get_perspective(self, points, counter):
        ...
        if len(points) != 1:
            ...
            img_size = np.array([[0, 0], [width_perspective - 1, 0], [width_perspective - 1, height_perspective -1], \
                [0, height_perspective - 1]], dtype = np.float32) #Whu float 32 it is converted to int
